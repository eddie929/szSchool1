    ...
    get_handledata({ leaderId: this.$route.query.data }).then((res) => {
      if (res.code > 0) {
        ...
        if (data && data.length > 0) {
          ...
          let printContent =
            "<div id='zuida' style=' width:520px;margin:0 auto;overflow:hidden;'>";
          ...
          for (let i = 0; i < pLength; i++) {
            ...
            printContent +=
              "<div id='zong' style='margin-top:42px;height:120px;font-size:8px;font-weight:bold;float:left;width:43%;margin-left:28%;background-color:#ecf5ff; '><h2 style='margin-top:-28px;margin-left:26px;text-align:center;font-size:14px;'>" +
              _self.data[i].学校名称 +
              "</h2><div  style='margin-top:28px;float:left;width:32%' id='XQ" +
              i +
              "'></div><div style='width:100%;margin-left:95px; '><p style='display:block;height:70px;margin-top:42px;font-size:15px;font-weight:500'>名称:" +
              _self.data[i].资产名称 +
              "</p><p style='margin-top:-32px;font-size:15px;font-weight:500'>编号:" +
              _self.data[i].资产编号缩写 +
              "</p><p style='margin-top:-12px;font-size:15px;font-weight:500'>购置日期:" +
              _self.data[i].购置日期 +
              "</p></div></div>";
          }
