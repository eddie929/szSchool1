<template>
  ...
  >
    ...
    >
      <el-button type="primary" class="el-icon-check" @click="okche()">
        撤 销</el-button
      >
    </div>
  </el-dialog>
    ...
    okche() {
      this.$confirm("是否撤销此次折旧?", "提示", {
        ...
        .then(() => {
          var data = {
            flowId: this.form.id,
            applicantId: this.form.申请人Id,
            departmenttwo: this.form.二级机构ID,
          };
           this.dialogVisible=false;
        //    return;
          depreciationdataithdrawremove(data).then((res)=>{
              this.dialogVisible=false;
              this.$emit("onload");
          })
        })
