  ...
  <el-dialog
    ...
    ><el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="处置单" name="first">
        ...
        <el-form ref="ruleForm" :label-position="labelPosition" :model="sform">
          ...
          <el-row :gutter="24" style="margin-top: 1%" v-if="form.流程代码 > 1">
            <el-col :span="8">
          ...
          <el-row :gutter="24" style="margin-top: 1%" v-if="form.流程代码 == 2">
            <el-col :span="8" v-if="form.流程代码 == 2">
              <el-form-item
                label="批复文号"
                label-width="140px"
                prop="批复文号"
    ...
    >
      ...
      <el-popconfirm
        ...
        style="margin-right: 20px"
      >
        <el-button class="el-icon-close" slot="reference">拒 绝</el-button>
      </el-popconfirm>
    </div>
  </el-dialog>
</template>
  ...
  methods: {
    ...
    btnok(val) {
      ...
      if (this.form.流程代码 == 1) {
        if (
          this.form.分管处理意见 == "" ||
          this.form.分管处理意见 == undefined
        ) {
          this.msg("警告", "请填写分管领导处理意见");
          return;
        }
        this.form.分管领导Id = this.$store.getters.id_用户;
        this.form.流程代码 = val;
        assetretirement_approval(this.form).then(res => {
          this.get_box(res.code, "审批成功!");
          this.$emit("onload");
          this.dialogVisible = false;
        });
      } else if (this.form.流程代码 == 2) {
        if (this.form.批复文号 == "" || this.form.批复文号 == undefined) {
          this.msg("警告", "请填写批复文号");
          return;
        }
        if (
          this.form.主管领导处理意见 == "" ||
          this.form.主管领导处理意见 == undefined
        ) {
          this.msg("警告", "请填写主管领导处理意见");
          return;
        }
        this.form.流程代码 = val;
        this.form.主管领导Id = this.$store.getters.id_用户;
        assetretirementleader_approval(this.form).then(res => {
          this.get_box(res.code, "审批成功!");
          this.$emit("onload");
          this.dialogVisible = false;
        });
