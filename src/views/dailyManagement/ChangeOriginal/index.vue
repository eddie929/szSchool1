    ...
    openAdd() {
      ...
      get_querytime(data).then(res => {
        if (res.data == '是') {
          ...
          this.$refs.AddChangeOriginal.form.一级机构ID = this.$store.getters.id_一级部门
          this.$refs.AddChangeOriginal.form.二级机构ID = this.$store.getters.id_二级部门
          this.$refs.AddChangeOriginal.form.申请人 = this.$store.getters.姓名
          this.$refs.AddChangeOriginal.form.申请人Id = this.$store.getters.id_用户
          this.$refs.AddChangeOriginal.form.申请日期 = this.dayjs().format(
            'YYYY-MM-DD'
          )
          this.$refs.AddChangeOriginal.dialogVisible = true
        }
