  ...
  <div>
    <div style="margin: 1% 1% 0% 1%">
      ...
      <div style="text-align: center; margin: 22px 0px">
        ...
        <el-popconfirm title="再次确认撤销" @confirm="btnok(3)" v-if="show">
          <el-button
            style="width: 10%"
            type="danger"
            class="el-icon-refresh-left"
        ...
        <!-- <el-popconfirm title="再次确认发起" @confirm="btnok(2)" v-if="hiden">
          <el-button
            style="width: 10%"
            type="primary"
            class="el-icon-check"
...
export default {
  ...
  methods: {
    back() {
      this.$router.push({ path: "/Transfer/index" });
    },
    btnok(str) {
      if (str === 3) {
        let data = {
        ...
        assetstransferapplicantremove(data).then(res => {
          if (res.code != 2) {
        ...
        // console.log("存放地点value",this.form.存放地点value);
        if (this.form.存放地点value === undefined) {
          this.form.存放地点value = [];
        }
        ...
        setTimeout(() => {
          if (this.form.id == 0 || this.form.id == undefined) {
            ...
            create_assetstransfer(this.form).then(res => {
              ...
              create_assetstransferdetail(this.tableData).then(res => {
                loading.close();
            ...
            let data = {
              ...
              id: this.form.id,
              备注信息: this.form.备注信息,
              负责人Id: this.form.负责人Id,
              存放地点一级Id: this.form.存放地点一级Id,
              存放地点Id: this.form.存放地点Id,
              归属部门Id: this.form.归属部门Id,
              流程代码: this.form.流程代码
            };
            assetstransfer_finaleapprovalsubmit(data).then(ress => {
    ...
    get_schoolresponseperson() {
      ...
      get_schoolresponseperson(data).then(res => {
        ...
        // console.log("负责人", this.负责人options);
  ...
  beforeMount() {
    ...
    this.form = {
      ...
      联系方式: this.$store.getters.手机
    };
    if (this.form.联系方式 === "") {
      this.form.联系方式 = "暂无联系方式";
    }
    this.tableData = [];
    ...
    if (this.$route.query.data === undefined) {
      ...
      if (
        ...
        this.form.归属部门value = {
          部门Id: this.form.归属部门Id,
          部门名称: this.form.归属部门
        };
        this.form.负责人value = {
          id: this.form.负责人Id,
          name: this.form.负责人名称
        };
        // this.get_storagelocation();
        // this.get_allschooldepartment();
        // this.get_schoolresponseperson();
