        ...
        if (this.$store.getters.职务 == "资产管理员") {
          if (str == -2) {
            if (this.form.id == "" || this.form.id == undefined) {
              ...
              // console.log("普通员工", this.form.维修单ID);
  ...
  beforeMount() {
    ...
    if (this.$route.query.data === undefined) {
      ...
      if (
        ...
        this.get_storagelocation();
        this.get_purchaseacceptancelistdetail(this.form.id);
        this.imageUrl = this.form.url;
        this.form.报修地址value = [
          this.form.报修地址Id一级,
          this.form.报修地址Id二级
          // 二级机构ID = this.$store.getters.id_二级部门,
          // 存放地点一级ID = this.form.报修地址Id一级,
          // 存放地点二级ID = this.form.报修地址Id二级,
          // 房间名称 = this.form.报修地址,
        ];
        this.hiden = true;
      }
