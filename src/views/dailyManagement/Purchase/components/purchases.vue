  ...
  <div>
    <div style="margin: 1% 1% 0% 1%">
      ...
      <div style="text-align: center; margin: 30px 0px">
        ...
        <Exltable ref="Exltable" @select="select" />
      </div>
    </div>
  </div>
</template>
...
import Exltable from "./Exltable.vue";
import {
...
export default {
  components: { Addasset, Extend, Exltable },
  data() {
    return {
      distable: false,
      distableexl: false,
  ...
  methods: {
        ...
        if (exl.length > 0) {
          for (var i = 0; i < exl.length; i++) {
            var list = {
              ...
              资产状态: exl[i].资产状态 + "",
              负责人: exl[i].负责人 + "",
              负责人name: exl[i].负责人name + "",
              使用人: exl[i].使用人 + "",
              使用人name: exl[i].使用人name + "",
              存放地点: exl[i].存放地点 + "",
              归属部门: exl[i].归属部门 + "",
              所属部门一级: exl[i].所属部门一级 + "",
              所属部门二级: exl[i].所属部门二级 + "",
            ...
            // 资产名称
            if (list.资产名称 == "undefined") {
              list.资产名称 = "";
            }
            // 一级分类名称
            if (list.一级分类名称 == "undefined") {
              list.一级分类名称 = "";
            }
            // 二级分类名称
            if (list.二级分类名称 == "undefined") {
              list.二级分类名称 = "";
            }
            // 三级分类名称
            if (list.三级分类名称 == "undefined") {
              list.三级分类名称 = "";
            }
            // 四级分类名称
            if (list.四级分类名称 == "undefined") {
              list.四级分类名称 = "";
            }
            // 规格
            if (list.规格 == "undefined") {
              list.规格 = "";
            }
            // 型号
            if (list.型号 == "undefined") {
              list.型号 = "";
            }
            // 数量
            if (list.数量 == "undefined") {
              list.数量 = "";
            }
            // 原值
            if (list.原值 == "undefined") {
              list.原值 = "";
            }
            // 使用方向
            if (list.使用方向 == "undefined") {
              list.使用方向 = "";
            }
            // 取得日期
            if (list.取得日期 == "undefined") {
              list.取得日期 = "";
            }
            // 资产状态
            if (list.资产状态 == "undefined") {
              list.资产状态 = "";
            }
            // 负责人
            if (list.负责人 == "undefined") {
              list.负责人 = "";
            }
            // 使用人
            if (list.使用人 == "undefined") {
              list.使用人 = "";
            }
            // 存放地点
            if (list.存放地点 == "undefined") {
              list.存放地点 = "";
            }
            // 所属部门一级
            if (list.所属部门一级 == "undefined") {
              list.所属部门一级 = "";
            }
            // 所属部门二级
            if (list.所属部门二级 == "undefined") {
              list.所属部门二级 = "";
            }
            // 房间名称
            if (list.房间名称 == "undefined") {
              list.房间名称 = "";
            }
            // 所属三级部门
            if (list.所属三级部门 == "undefined") {
              list.所属三级部门 = "";
            }
            exltData.push(list);
          }
          for (let i of exltData) {
            i.资产分类 =
              i.一级分类名称 +
              " " +
              i.二级分类名称 +
          ...
          this.$refs.Exltable.tableData = exltData;
          this.$refs.Exltable.dialogVisible = true;
        } else {
          this.msg("警告", "不可以上传空数据!");
        }
      };
    ...
    downloadImportTemplate() {
      ...
      a.href = "http://82.156.97.125:8081/资产导入模板.xls";
      a.click();
    },
    back() {
    ...
    edit(row, index) {
      ...
      collRow.资产合并 = collRow.资产分类[0]+"/"+collRow.资产分类[1]+"/"+collRow.资产分类[2]+"/"+collRow.资产分类[3]
