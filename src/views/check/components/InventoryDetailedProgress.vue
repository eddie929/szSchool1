...
import {
  ...
  get_eductiontobecountedbyflowstate,
  get_eductiontobecounted
  ...
  methods: {
    ...
    onload() {
      ...
      let reqdata = {
        ...
        departmenttwo: this.pointPerson.fk_user
      };
      const loading = this.$loading({
        lock: true,
        text: "Loading",
      ...
      if (this.$store.getters.id_二级部门 == 0) {
        if (this.isCheck == "已盘点") {
          this.isShow = false;
          get_eductiontobecountedbyflowstate(reqdata)
            .then(res => {
              // console.table("已盘点");
          ...
          this.isShow = true;
          get_eductiontobecounted(reqdata)
            .then(res => {
              // console.table("未盘点");
              // console.table(res.data);
              // console.log(res);
